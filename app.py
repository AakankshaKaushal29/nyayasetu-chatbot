...
if voice_data and "bytes" in voice_data:
    try:
        # Convert WebM/Opus bytes to WAV using pydub
        audio_bytes = io.BytesIO(voice_data["bytes"])
        audio = AudioSegment.from_file(audio_bytes, format="webm")  # or "ogg" if needed
        wav_io = io.BytesIO()
...
if user_query and user_query.strip():
    ...
    # Step-by-step with clean alignment, all text on one line
    steps = get_detailed_steps(best_match[answer_col], desired=6)
    st.markdown("### ✅ Step-by-Step Guidance:")
    for i, step in enumerate(steps):
        clean_step = ' '.join(step.splitlines())  # remove internal line breaks
