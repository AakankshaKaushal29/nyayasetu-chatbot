...
if submitted and user_question:
    ...
    if col_name in df.columns:
        ...
        if score > 50 and idx is not None:  # lower threshold to allow variations
    ...
    if client and detailed_answer not in ["", short_answer]:
        try:
            ...
            detailed_answer = ai_answer  # replace with AI-polished answer
        except Exception as e:
            st.warning("⚠️ AI enhancement failed, showing dataset answer only.")

    # Show answers in a card-like format
    st.markdown("### 📖 Answers")
    ...
    if selected_lang in ["English", "Hindi"]:
        ...
        with col2:
            if st.button("🔉 Play Detailed Answer"):
                try:
                    tts = gTTS(text=detailed_answer, lang="hi" if selected_lang=="Hindi" else "en")
                    with tempfile.NamedTemporaryFile(delete=True) as fp:
                        tts.save(f"{fp.name}.mp3")
                        st.audio(f"{fp.name}.mp3")
                except:
